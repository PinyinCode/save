...
- Thông báo khi user bấm "Tôi đã thanh toán"
- Thông báo khi admin xác nhận gia hạn
...
/* ═══════════════════════════════════════════════════════════════
   TELEGRAM NOTIFICATION (THÊM MỚI — không ảnh hưởng app nếu lỗi)
   ═══════════════════════════════════════════════════════════════ */
function sendTelegramMessage(text) {
    try {
        var token = (typeof TELEGRAM_BOT_TOKEN !== 'undefined') ? TELEGRAM_BOT_TOKEN : '';
        var chatId = (typeof TELEGRAM_CHAT_ID !== 'undefined') ? TELEGRAM_CHAT_ID : '';
        if (!token || !chatId) {
        ...
        fetch('https://api.telegram.org/bot' + token + '/sendMessage', {
            ...
            if (d.ok) console.log('✅ Đã gửi Telegram');
            else console.warn('⚠️ Telegram lỗi:', d.description);
        }).catch(function(e) { console.error('❌ Telegram fetch lỗi:', e); });
...
function notifyTelegramUserPaid(reqData) {
    ...
    } catch(e) { console.error('❌ notifyTelegramUserPaid error:', e); }
}
...
function notifyTelegramAdminConfirmed(reqData, newExpiry) {
    try {
        ...
        msg += '🕐 <b>Lúc:</b> ' + timeStr;
        sendTelegramMessage(msg);
    } catch(e) { console.error('❌ notifyTelegramAdminConfirmed error:', e); }
}
...
window.userConfirmPaid = async function() {
    ...
    try {
        /* TELEGRAM: lấy data trước khi update */
        var _reqData = null;
        try {
        ...
        /* TELEGRAM: gửi thông báo (không await, không block UI) */
        try {
            if (_reqData && typeof notifyTelegramUserPaid === 'function') {
...
window.approveRenewal = async function(reqId) {
    ...
    try {
        ...
        /* TELEGRAM: gửi thông báo admin đã xác nhận */
        try {
            if (typeof notifyTelegramAdminConfirmed === 'function') {
                var _newExpiryStr = _newExpiryDate ? _newExpiryDate.toLocaleDateString('vi-VN') : '';
