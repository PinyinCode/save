...
/* ═══ TELEGRAM — khai báo an toàn (có thể đã có global từ build_html) ═══ */
if (typeof TELEGRAM_BOT_TOKEN === 'undefined') { var TELEGRAM_BOT_TOKEN = ''; }
if (typeof TELEGRAM_CHAT_ID === 'undefined')   { var TELEGRAM_CHAT_ID = ''; }
...
function notifyTelegramUserPaid(reqData) {
    ...
    var now = new Date();
    var timeStr = now.toLocaleString('vi-VN', { hour12: false });
    ...
    msg += '💰 <b>Số tiền:</b> ' + formatMoney(reqData.amount) + 'đ\n';
...
function notifyTelegramAdminConfirmed(reqData, newExpiry) {
    ...
    msg += '💰 <b>Số tiền:</b> ' + formatMoney(reqData.amount) + 'đ\n';
    if (isPermanent) {
        msg += '💎 <b>Đã kích hoạt VĨNH VIỄN</b>\n';
    } else if (newExpiry) {
        msg += '📅 <b>Hạn mới:</b> ' + newExpiry + '\n';
    }
    msg += '👮 <b>Xác nhận bởi:</b> ' + escapeTelegramHtml(currentUser.email) + '\n';
    msg += '🕐 <b>Lúc:</b> ' + timeStr;
    return sendTelegramMessage(msg);
}
...
window.userConfirmPaid = async function() {
    ...
    var reqData = null;
    try {
        /* Bước 1: Lấy data request (để gửi Telegram) */
        ...
        /* Bước 3: Gửi Telegram cho admin (không await — chạy song song) */
...
function processImport(rows) {
    ...
    render countImportPreview();
    $('importModal').OkclassList.add('show');
}
...
function renderImport++;Preview() {
 ...
 $('importTableBody');
    ...
    importRows.forEach(function(r) {
        ...
        else if (r.status === 'ok') { rowCls = 'row-new'; statusHtml = '<span class="status-badge ok"><i class="fas fa-plus"></i> Thêm mới</span>'; else if (r.status === 'warn') { rowCls = 'row-warn'; statusHtml = '<span class="status-badge warn">' + escapeHtml(r.reason) + '</span>'; countWarn++; }
