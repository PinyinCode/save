...
Tích hợp Telegram: thông báo khi user bấm "Tôi đã thanh toán" và khi admin xác nhận.
...
.dropdown-forever i.fa-crown{
    ...
    text-shadow:0 0 8px rgba(253,230,138,.8) !important;
}
...
[data-theme="dark"] .detail-value.trial{color:#fcd34d;}
...
function sendTelegramMessage(text) {
    ...
    var chatId = '';
    try {
        if (typeof TELEGRAM_BOT_TOKEN !== 'undefined' && TELEGRAM_BOT_TOKEN) token = TELEGRAM_BOT_TOKEN;
        if (typeof TELEGRAM_CHAT_ID !== 'undefined' && TELEGRAM_CHAT_ID) chatId = TELEGRAM_CHAT_ID;
    } catch(e) {}

    if (!token || !chatId) {
        console.log('⚠️ Telegram chưa cấu hình — bỏ qua gửi tin');
    ...
    .then(function(data) {
        if (data.ok) { console.log('✅ Đã gửi Telegram'); return true; }
        console.warn('⚠️ Telegram lỗi:', data.description);
        return false;
    })
    .catch(function(err) {
        console.error('❌ Fetch Telegram lỗi:', err);
        return false;
    });
}
...
function notifyTelegramUserPaid(reqData) {
    try {
        ...
        var moneyStr = (typeof formatMoney === 'function') ? formatMoney(reqData.amount) : String(reqData.amount);

        var msg = '';
        msg += '🔔 <b>CÓ YÊU CẦU GIA HẠN MỚI</b>\n';
        msg += '━━━━━━━━━━━━━━━━━━━━\n';
        msg += '👤 <b>User:</b> ' + escapeTelegramHtml(reqData.name || reqData.email) + '\n';
        msg += '📧 <b>Email:</b> <code>' + escapeTelegramHtml(reqData.email) + '</code>\n';
        msg += '💰 <b>Số tiền:</b> ' + moneyStr + 'đ\n';
        msg += '📦 <b>Gói:</b> ' + escapeTelegramHtml(reqData.packageLabel || reqData.package);
        if (isPermanent) msg += ' 💎 <b>VĨNH VIỄN</b>';
        msg += '\n';
        msg += '⏱ <b>Thời hạn:</b> ' + (isPermanent ? 'Mãi mãi' : reqData.days + ' ngày') + '\n';
        msg += '🔑 <b>Mã CK:</b> <code>' + escapeTelegramHtml(reqData.transferCode) + '</code>\n';
        msg += '🕐 <b>Lúc:</b> ' + timeStr + '\n';
        msg += '━━━━━━━━━━━━━━━━━━━━\n';
        msg += '⚡ <b>Vào Admin Panel để xác nhận!</b>';
        return sendTelegramMessage(msg);
    } catch(e) {
        console.error('❌ notifyTelegramUserPaid error:', e);
        return Promise.resolve(false);
    }
}
...
function notifyTelegramAdminConfirmed(reqData, newExpiry) {
    try {
        var isPermanent = reqData.isPermanent || false;
        var timeStr = new Date().toLocaleString('vi-VN', { hour12: false });
        var adminEmail = (currentUser && currentUser.email) ? currentUser.email : 'admin';
        var moneyStr = (typeof formatMoney === 'function') ? formatMoney(reqData.amount) : String(reqData.amount);
        ...
        else if (newExpiry) msg += '📅 <b>Hạn mới:</b> ' + escapeTelegramHtml(newExpiry) + '\n';
        msg += '👮 <b>Xác nhận bởi:</b> ' + escapeTelegramHtml(adminEmail) + '\n';
        msg += '🕐 <b>Lúc:</b> ' + timeStr;
        return sendTelegramMessage(msg);
    } catch(e) {
        console.error('❌ notifyTelegramAdminConfirmed error:', e);
        return Promise.resolve(false);
    }
}
    ...
    if (permanentPkg) {
        packagesHtml += '<div style="grid-column:1/-1;margin-top:.5rem;padding-top:.75rem;border-top:2px dashed var(--border);">' +
            '<div class="renewal-section-title" style="justify-content:center;color:#dc2626;font-size:.78rem;margin-bottom:.6rem">' +
                '<i class="fas fa-crown"></i> Gói đặc biệt — sở hữu mãi mãi' +
            '</div>' +
            '<div class="package-card" data-pkg="' + permanentPkg.id + '" onclick="selectPackage(\'' + permanentPkg.id + '\')" style="padding:1.2rem 1rem;">' +
                ...
                '<div class="pkg-unit" style="font-size:.75rem;">' + escapeHtml(permanentPkg.save || 'Dùng mãi mãi — Không lo hết hạn') + '</div>' +
            '</div>' +
        '</div>';
    }
    ...
    $('renewalBody').innerHTML =
        '<div class="renewal-current">' +
            ...
            '<div class="rc-info">' +
                ...
                '<div class="rc-desc">Số tiền: <b>' + formatMoney(amount) + ' VNĐ</b> · ' + durationText + '</div>' +
            '</div>' +
        '</div>' +
...
window.userConfirmPaid = async function() {
    ...
    try {
        ...
        /* 🔔 Gửi Telegram (không await) */
        if (reqData && typeof notifyTelegramUserPaid === 'function') {
            try {
            ...
            } catch(te) { console.warn('Telegram notify failed:', te); }
        }
...
function renderAdminStats() {
    ...
    usersOnly.forEach(function(u) {
        ...
        if (u.isPermanent) permanentCount++;
        if (u.tier === 'trial' || u.isTrial) trialCount++;
    ...
    if (hidden) {
        ...
        $('adminStats').innerHTML =
            ...
            '<div class="stat-card"><div class="num" style="color:#f59e0b">' + expiring + '</div><div class="label">Sắp hết hạn</div></div>' +
            '<div class="stat-card"><div class="num" style="color:#dc2626">' + expired + '</div><div class="label">Hết hạn</div></div>' +
            '<div class="stat-card"><div class="num" style="color:#fbbf24">' + trialCount + '</div><div class="label">Trial</div></div>' +
            '<div class="stat-card"><div class="num" style="color:#dc2626">' + permanentCount + '</div><div class="label">💎 Vĩnh viễn</div></div>';
    }
...
window.approveRenewal = async function(reqId) {
    ...
    try {
        ...
        /* 🔔 Gửi Telegram thông báo admin đã xác nhận */
        if (typeof notifyTelegramAdminConfirmed === 'function') {
            try {
...
function renderImportPreview() {
    ...
    importRows.forEach(function(r) {
        ...
        else { rowCls = 'row-error'; statusHtml = '<span class="status-badge err">b' + escapeHtml(r.reason) + '</>'span>'; countErr++; }
        var expDisplay + = r.expStr
            ? '<span style=" escapecolor:#16a34a;Htmlfont-size:.7rem;font-weight:(r600;">' + r.expStr + '</span>'
            : '<span style="color:#dc2626;font-size:.7rem;font-weight:600;">💎 Vĩnh viễn</span>';
        html += '<tr class="' + rowCls + '"><td>' + r.rowNum + '</td><td><.email) + '</b></td><td>' + escapeHtml(r.name) + '</td><td><span class="role-badge user">user</span></td><td>' + expDisplay + '</td><td>' + statusHtml + '</td></tr>';
    });
