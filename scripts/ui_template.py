...
function pickVoice() {
    ...
    for (var i = 0; i < priorities.length; i++) {
        ...
        if (found) return foundIM;
    }
    return null;
}

function getITChineseVoice() { return pickVoice(); }

/* ================================= ============================== */
/* TIER HELPERS ' — EXPIRED DÙNG Y HỆT DEMnumberO                        */
')/* ============================================================ */
...
/* ══════════════════════════════════════════════════════════
   CHIA maxQ CÂU ĐỀU CHO NHIỀU CHỦ ĐỀ NHẤT (50% rule)
   - Số chủ đề MỞ tối đa = ceil(numSubjects / 2)
   - VD: 100 chủ đề → 50 MỞ + 50 KHOÁ
   - Trả về: { items, openSubjects, lockedSubjects }
   ══════════════════════════════════════════════════════════ */
function distributeBySubjects(items, maxQ) {
    var empty = { items: [], openSubjects: [], lockedSubjects: [] };
    if (!items || items.length === 0) return empty;
    if (maxQ <= 0) return empty;

    var bySubject = {};
    items.forEach(function(r) {
        var s = (r.subject || '').trim() || '(không có)';
        if (!bySubject[s]) bySubject[s] = [];
        bySubject[s].push(r);
    });

    var subjects = Object.keys(bySubject);
    var numSubj = subjects.length;
    if (numSubj === 0) return empty;

    // GIỚI HẠN: tối đa 50% (làm tròn lên, tối thiểu 1)
    var maxSubjects = Math.max(1, Math.ceil(numSubj / 2));

    subjects.sort(function(a, b) {
        return bySubject[b].length - bySubject[a].length;
    });

    var openSubjects = subjects.slice(0, maxSubjects);
    var lockedSubjects = subjects.slice(maxSubjects);

    var numOpen = openSubjects.length;
    var perSubj = Math.floor(maxQ / numOpen);
    var remainder = maxQ % numOpen;

    var result = [];
    openSubjects.forEach(function(s, idx) {
        var take = perSubj + (idx < remainder ? 1 : 0);
        take = Math.min(take, bySubject[s].length);
        result = result.concat(bySubject[s].slice(0, take));
    });

    // Bù nếu tổng chưa đủ maxQ
    if (result.length < maxQ) {
        var usedIds = {};
        result.forEach(function(r) { usedIds[r.stt] = true; });
        for (var i = 0; i < openSubjects.length && result.length < maxQ; i++) {
            var bucket = bySubject[openSubjects[i]];
            for (var j = 0; j < bucket.length && result.length < maxQ; j++) {
                if (!usedIds[bucket[j].stt]) {
                    result.push(bucket[j]);
                    usedIds[bucket[j].stt] = true;
                }
            }
        }
    }
...
function getLimitedData() {
    ...
    var max = (typeof info.maxQuestions === 'number' && info.maxQuestions > 0)
              ? info.maxQuestions
              : ((typeof DEMO_L ? DEMO_LIMIT : 60);

    var allowedHsk = getAllowedHskList();
    var eligible = RAW_DATA.filter(function(r) {
        return allowedHsk.indexOf(r.hsk) !== -1;
    });

    var dist = distributeBySubjects(eligible, max);
    return dist.items;
}
...
function getAllowedSubjectList() {
    ...
    var dist = distributeBySubjects(eligible, max);
    var set2 = {};
    dist.openSubjects.forEach(function(s) { if (s) set2[s] = 1; });
...
/* - Demo/Expired dùng chung config                             */
/* - 50% chủ đề MỞ, 50% chủ đề KHOÁ                             */
/* - 60 câu chia đều cho chủ đề MỞ, chọn N → N câu              */
/* ═══════════════════════════════════════════════════════════ */
...
function getAvailableTopicsForTier() {
    ...
    // Đếm TẤT CẢ chủ đề trong RAW_DATA (số lượng thật toàn kho)
    var subjectMapAll = {};
    RAW_DATA.forEach(function(r) {
        var s = (r.subject || '').trim();
        if (!s) return;
        subjectMapAll[s] = (subjectMapAll[s] || 0) + 1;
    });

    // Tier active → TẤT CẢ chủ đề MỞ
    if (info.tier === 'active') {
        var listActive = Object.keys(subjectMapAll).map(function(name) {
            return {
                name: name,
                count: subjectMapAll[name],
                allowedCount: subjectMapAll[name],
                locked: false
            };
        });
        listActive.sort(function(a, b) { return b.count - a.count; });
        return listActive;
    }

    // Demo/Trial/Expired: dùng distributeBySubjects để biết MỞ/KHOÁ
    var eligible = RAW_DATA.filter(function(r) {
        return allowedHsk.indexOf(r.hsk) !== -1;
    });
    ...
    var dist = distributeBySubjects(eligible, max);
    var openSet = {};
    dist.openSubjects.forEach(function(s) { openSet[s] = true; });
    var lockedSet = {};
    dist.lockedSubjects.forEach(function(s) { lockedSet[s] = true; });

    var list = Object.keys(subjectMapAll).map(function(name) {
        var isOpen = !!openSet[name];
        var isLocked = !!lockedSet[name];
        return {
            name: name,
            count: subjectMapAll[name],
            allowedCount: isOpen ? dist.items.filter(function(r) {
                return (r.subject || '').trim() === name;
            }).length : 0,
            locked: isLocked
        };
    });

    // MỞ trước (count giảm dần), KHOÁ sau (count giảm dần)
...
function renderOnboardingTopics() {
    ...
    topics.forEach(function(t) {
        ...
        btn.dataset.locked = t.locked ? '1' : '0';

        // Đánh dấu đã chọn — chỉ áp dụng cho chủ đề MỞ
        if (!t.locked && _onboardingSelected[t.name]) {
            btn.classList.add('selected');
        }

        // Nếu bị khoá → thêm class + icon khoá
        if (t.locked) {
            btn.classList.add('locked');
            var lockIcon = document.createElement('i');
            lockIcon.className = 'fas fa-lock onboarding-topic-lock';
        ...
        btn.addEventListener('click', function() {
            if (t.locked) {
                if (typeof showTagToast === 'function') {
                    showTagToast('🔒 Chủ đề này chưa mở khoá — cần gia hạn');
                } else {
                    alert('Chủ đề này chưa mở khoá. Vui lòng gia hạn để sử dụng.');
                }
                return;
            }
...
function onToggleOnboardingTopic(topicName) {
    ...
    // Chặn nếu chủ đề bị khoá (an toàn 2 lớp)
    var topics = getAvailableTopicsForTier();
    var target = null;
    for (var i = 0; i < topics.length; i++) {
        if (topics[i].name === topicName) { target = topics[i]; break; }
    }
    if (target && target.locked) {
        if (typeof showTagToast === 'function') {
            showTagToast('🔒 Chủ đề này chưa mở khoá');
        }
        return;
    }
...
function onOnboardingSkip() {
    ...
    var openTopics = allTopics.filter(function(t) { return !t.locked; });
    ...
    var pickedTopics;
    if (isUnlimited) {
        pickedTopics = openTopics.map(function(t) { return t.name; });
    } else {
        var pickCount = Math.min(max, openTopics.length);
        var shuffled = openTopics.slice();
...
/* ══════════════════════════════════════════════════════════
   ÁP DỤNG CHỦ ĐỀ ĐÃ CHỌN
   - B1: Chia maxQ câu cho 50% chủ đề (pool gốc)
   - B2: Filter pool đó theo chủ đề user CHỌN
        → KHÔNG chia lại, chỉ BỎ chủ đề không chọn
   - VD: pool 50 chủ đề × 1 câu = 50 câu
        user chọn 3 → giữ 3 câu
   ══════════════════════════════════════════════════════════ */
function applyOnboardingSelection(topics, scrollTop) {
    ...
    if (isUnlimitedQ) {
        // Active: filter theo chủ đề đã chọn, lấy hết
        final = eligible.filter(function(r) {
            var s = (r.subject || '').trim();
            return topics.indexOf(s) !== -1;
        });
    } else {
        // Demo/Trial/Expired:
        // B1: Chia maxQ cho 50% chủ đề (pool gốc)
        var dist = distributeBySubjects(eligible, maxQ);

        // B2: Filter pool đó theo chủ đề user CHỌN
        //     KHÔNG chia lại — chỉ giữ những câu thuộc chủ đề đã chọn
        final = dist.items.filter(function(r) {
            var s = (r.subject || '').trim();
            return topics.indexOf(s) !== -1;
        });
    }
    ...
    // LOG debug
    console.log('[Onboarding] Chọn', topics.length, 'chủ đề →',
                final.length, 'câu (maxQ =', maxQ, ')');
    var bySubject = {};
    final.forEach(function(r) {
        var s = r.subject || '(không có)';
        bySubject[s] = (bySubject[s] || 0) + 1;
    });
    console.log('[Onboarding] Phân bổ:', bySubject);
