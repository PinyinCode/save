...
✅ QUY TẮC:
   ...
   - Chế độ Full không chồng lấn: dùng flex column,
     header/filters/nav flex: 0 0 auto, body flex: 1 1 auto + overflow-y: auto.
...
✅ MINI BRAND trong Practice Full Header:
   ...
   - Cân đối bên trái, counter + nút X bên phải
   - ★ Mobile GIỐNG PC: subtitle KHÔNG ẩn, chỉ scale nhỏ

✅ TIKTOK FLOATING PILL:
   - Ở GÓC DƯỚI BÊN TRÁI (fixed)
   - Hiển thị: avatar + label "THEO DÕI" + tên (VD: "Thảo Nói") + badge 🎵
   - Tự ẩn khi modal đóng, tự ẩn nếu TIKTOK_URL rỗng
"""
...
/* ═══════════════════════════════════════════════════════════════
   ★★★ PRACTICE FULL MODAL — KHÔNG CHỒNG LẤN ★★★
   ═══════════════════════════════════════════════════════════════ */
.practice-full-modal{
    position:fixed;
    inset:0;
...
.practice-full-header .pf-counter{
    font-size:clamp(.7rem,.9vw,.82rem);font-weight:700;color:var(--text-2);
    background:var(--surface-2);padding:.3rem .7rem;border-radius:50px;
...
/* ─── Responsive: thu nhỏ nhưng KHÔNG ẩn ─── */
...
/* ─── TIKTOK FLOATING PILL (avatar + tên) — GÓC DƯỚI BÊN TRÁI ─── */
.pf-tiktok-float{
    position:fixed;
    left:clamp(14px,2vw,22px);              /* ★ GÓC DƯỚI BÊN TRÁI */
    bottom:calc(clamp(72px,10vh,96px) + env(safe-area-inset-bottom));
    display:inline-flex;
    align-items:center;
...
.pf-tiktok-avatar[src]:not([src=""]) + .pf-tiktok-fallback-icon{
    display:none;
}
...
/* ─── Ẩn khi modal đóng ─── */
.practice-full-modal:not(.show) .pf-tiktok-float{
...
.practice-full-body{
    ...
    min-height:0;
    overflow-y:auto;
    padding:clamp(.85rem,2vh,1.5rem) clamp(.75rem,2vw,1.25rem);
    display:flex;
    flex-direction:column;
    align-items:center;
...
@media(max-width:768px){
    ...
    .practice-full-body{padding:.85rem .7rem}
...
@media(max-width:400px){
    .practice-full-body{padding:.7rem .5rem}
...
function initPracticeFull() {
    ...
    try {
        ...
        if (pfTiktok) {
            ...
            if (tiktokUrl) {
                ...
                /* Tên: ưu tiên NICKNAME (VD: "Thảo Nói") */
                var displayName = '';
    ...
    } catch(e) { console.warn('TikTok float init:', e); }
