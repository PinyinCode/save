...
.practice-full-header .pf-counter{
    font-size:clamp(.68rem,.88vw,.8rem);font-weight:800;color:#fff;
    background:linear-gradient(135deg,#4f46e5,#7c3aed);
...
.practice-full-header .pf-tags .card-tag{
    flex-shrink:0;
    font-weight:700;
}
...
.pf-brand-icon{
    ...
    border-radius:clamp(8px,.9vw,11px);
    background:linear-gradient(135deg,#6366f1 0%,#8b5cf6 40%,#d946ef 100%);
    display:flex;align-items:center;justify-content:center;
    color:#fff;font-size:clamp(.78rem,1vw,.95rem);
    flex-shrink:0;
    box-shadow:0 3px 10px rgba(139,92,246,.35), inset 0 1px 0 rgba(255,255,255,.25);
}
.pf-brand-text{
    display:flex;flex-direction:column;line-height:1.15;
    min-width:0;overflow:hidden;gap:2px;
}
.pf-brand-title{
    font-size:clamp(.8rem,1vw,.95rem);font-weight:900;letter-spacing:-.015em;
    background:linear-gradient(135deg,#1e293b 0%,#4f46e5 50%,#7c3aed 100%);
    -webkit-background-clip:text;background-clip:text;
    -webkit-text-fill-color:transparent;color:transparent;
    white-space:nowrap;
}
[data-theme="dark"] .pf-brand-title{
...
.pf-brand-sub{
    ...
    font-size:clamp(.58rem,.72vw,.68rem);font-weight:700;
    color:#5b21b6;letter-spacing:.01em;
    white-space:nowrap;overflow:hidden;text-overflow:ellipsis;
}
.pf-brand-sub::before{
    content:'✦';color:#d946ef;font-size:.95em;
    text-shadow:0 0 5px rgba(217,70,239,.7),0 0 10px rgba(139,92,246,.5);
    animation:sparklePF 2.5s ease-in-out infinite;
}
@keyframes sparklePF{
...
@media (max-width: 768px) {
    .pf-brand-title { font-size: .75rem; }
    .pf-brand-sub { font-size: .55rem; gap: .2rem; }
    .pf-brand-icon {
...
@media (max-width: 400px) {
    .pf-brand-title { font-size: .7rem; }
    .pf-brand-sub { font-size: .5rem; }
    .pf-brand-icon {
...
.pf-tiktok-float{
    ...
    display:inline-flex;
    align-items:center;
    gap:clamp(.4rem,.6vw,.55rem);
    padding:clamp(.35rem,.55vw,.5rem) clamp(.7rem,1vw,.9rem) clamp(.35rem,.55vw,.5rem) clamp(.35rem,.55vw,.5rem);
    border-radius:999px;
...
.pf-tiktok-float::before{
    content:'';
    position:absolute;
    inset:0;
    background:linear-gradient(115deg, transparent 30%, rgba(255,255,255,.35) 50%, transparent 70%);
    transform:translateX(-100%);
    animation:shimmerTikTok 3s infinite;
    pointer-events:none;
}
@keyframes shimmerTikTok{
    0%{transform:translateX(-100%);}
    60%,100%{transform:translateX(100%);}
}
.pf-tiktok-float:hover{
    transform:translateY(-3px) scale(1.04);
    box-shadow:
        0 14px 36px rgba(254,44,85,.6),
        0 6px 20px rgba(0,0,0,.45),
        0 0 0 3px rgba(37,244,238,.5),
        inset 0 1px 0 rgba(255,255,255,.3);
}
.pf-tiktok-float:active{transform:scale(.97);}
.pf-tiktok-avatar-wrap{
    position:relative;
...
.pf-tiktok-fallback-icon{
    position:absolute;inset:0;
    display:flex;align-items:center;justify-content:center;
    color:#fff;font-size:clamp(.9rem,1.1vw,1.1rem);
...
.pf-tiktok-content{
    ...
    line-height:1.1;min-width:0;flex:1;text-align:left;
}
.pf-tiktok-label{
    font-size:clamp(.5rem,.62vw,.6rem);
    font-weight:600;opacity:.8;letter-spacing:.4px;
...
.pf-tiktok-badge{
    ...
    flex-shrink:0;
    filter:drop-shadow(0 0 4px rgba(37,244,238,.6));
...
@media (max-width:500px){
    ...
    .pf-tiktok-avatar-wrap{width:26px;height:26px;}
    .pf-tiktok-label{font-size:.5rem;}
    .pf-tiktok-name{font-size:.68rem;}
    .pf-tiktok-badge{font-size:.8rem;}
}
...
.pf-search-wrap input{
    width:100%;padding:clamp(.35rem,.6vh,.55rem) 2.3rem clamp(.35rem,.6vh,.55rem) 2.3rem;
    border-radius:var(--radius-full);border:1.5px solid var(--border);
...
.pf-search-clear{
    ...
    background:var(--surface-2);color:var(--text-2);
    cursor:pointer;display:none;align-items:center;justify-content:center;font-size:.72rem;
}
...
.pf-chip{
    display:flex;align-items:center;gap:.4rem;
    padding:clamp(.25rem,.5vh,.4rem) clamp(.5rem,.8vw,.75rem);
    border-radius:var(--radius-full);
    border:1.5px solid var(--border);background:var(--bg);
    color:var(--text);font-size:clamp(.68rem,.85vw,.78rem);font-weight:500;
    cursor:pointer;transition:.15s;outline:none;font-family:inherit;
...
.pf-chip-label{font-size:clamp(.55rem,.7vw,.62rem);color:var(--text-3);text-transform:uppercase;letter-spacing:.3px;font-weight:700;flex-shrink:0}
.pf-chip-value{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0}
.pf-chip-arrow{color:inherit;opacity:.5;font-size:.62rem;flex-shrink:0}
.pf-chip select{
    position:absolute;inset:0;opacity:0;cursor:pointer;font-size:1rem;
...
.pf-result-count{
    display:none;align-items:center;gap:.4rem;margin-top:.4rem;
    padding:.3rem .65rem;border-radius:var(--radius-full);
    background:var(--bg);border:1px solid var(--border);
    color:var(--text-2);font-size:clamp(.62rem,.8vw,.72rem);font-weight:600;
    width:fit-content;
}
.pf-result-count.show{display:inline-flex}
.pf-result-count i{color:var(--primary);font-size:.78rem}
...
.pf-quick-nav-label{
    font-size:clamp(.55rem,.72vw,.68rem);font-weight:700;color:var(--text-3);
    text-transform:uppercase;letter-spacing:.3px;white-space:nowrap;
}
.pf-quick-nav-select{
    flex:1;min-width:0;
    padding:clamp(.25rem,.5vh,.45rem) 2rem clamp(.25rem,.5vh,.45rem) clamp(.5rem,.8vw,.75rem);
    border-radius:var(--radius-full);border:1.5px solid var(--border);
    background:var(--bg);color:var(--text);
...
.practice-full-body{
    ...
    min-height:0;
    overflow-y:auto;
    padding:clamp(.85rem,2vh,1.5rem) clamp(.75rem,2vw,1.25rem) calc(80px + env(safe-area-inset-bottom)) clamp(.75rem,2vw,1.25rem);
    display:flex;
    flex-direction:column;
    align-items:center;
...
@media(min-width:1400px){
    .practice-full-input{font-size:1.7rem;padding:1.05rem 1.4rem;}
}
...
.practice-speak-btn:disabled{opacity:.4;cursor:not-allowed;transform:none;}
...
@media (min-width: 769px) and (max-height: 700px) {
    ...
    .practice-full-nav{ padding: .5rem .85rem; }
...
@media (min-width: 769px) and (max-height: 550px) {
    ...
    .pf-filters{ padding: .25rem .85rem .2rem .85rem; }
    .pf-search-wrap{ display: none; }
...
@media(max-width:768px){
    ...
    .practice-full-header{padding:.5rem .85rem;gap:.5rem}
    .pf-filters{padding:.4rem .85rem .3rem .85rem}
    .practice-full-body{padding:.85rem .7rem calc(80px + env(safe-area-inset-bottom)) .7rem}
    ...
    .practice-full-nav{padding:.55rem .85rem;gap:.5rem}
    .reveal-actions button{padding:.6rem;font-size:.8rem;}
    .reveal-actions{grid-template-columns:1fr 1fr;gap:.5rem;}
}
@media(max-width:400px){
    .practice-full-body{padding:.7rem .5rem calc(80px + env(safe-area-inset-bottom)) .5rem}
    .practice-full-content{gap:.7rem}
...
.practice-full-header .card-tag.hsk{
    background:linear-gradient(135deg,#4f46e5,#7c3aed);
    color:#fff;
    box-shadow:0 2px 6px rgba(124,58,237,.3);
}
.practice-full-header .card-tag.topic{
    background:linear-gradient(135deg,#f59e0b,#d97706);
    color:#fff;
    box-shadow:0 2px 6px rgba(245,158,11,.3);
}
.practice-full-header .card-tag:not(.hsk):not(.topic){
    background:var(--surface-2);
    color:var(--text-2);
    border:1px solid var(--border);
}
[data-theme="dark"] .practice-full-header .card-tag:not(.hsk):not(.topic){
    background:var(--surface-2);
    color:var(--text);
}
...
.practice-full-header .card-tag.excel-tag i{
    font-size:.85em;
}
...
@media (min-width: 769px) {
    ...
    .header-inner .tiktok-bar{
        ...
        min-width:0 !important;
        max-width:clamp(140px,22vw,260px) !important;
        margin:0 !important;
    ...
    .header-inner .tiktok-bar .tiktok-bar-link i{
        font-size:clamp(.7rem,.85vw,.8rem) !important;
    }
    ...
    @media (max-width:1200px){
        .header-inner .tiktok-bar{ max-width:clamp(120px,18vw,200px) !important; }
    }
    @media (max-width:1024px){
        .header-inner .tiktok-bar{ max-width:clamp(100px,15vw,160px) !important; }
        .header-inner .tiktok-bar .tiktok-user{ display:none; }
    }
    @media (max-width:900px){
        .header-inner .tiktok-bar{ max-width:130px !important; }
        .header-inner .tiktok-bar .tiktok-bar-link span{ display:none; }
        .header-inner .tiktok-bar .tiktok-bar-link{
...
<div class="practice-full-modal" id="practiceFullModal">
    <div class="practice-full-header">
        <div class="pf-brand">
            <div class="pf-brand-icon"><i class="fas fa-language"></i></div>
            <div class="pf-brand-text">
                <div class="pf-brand-title">Học tiếng Trung</div>
                <div class="pf-brand-sub">Văn phòng &amp; Công xưởng</div>
            </div>
        </div>
...
/* ═══════════════════════════════════════════════════════════════
   ★ DI CHUYỂN TIKTOK BAR VÀO HEADER — CHỈ DESKTOP (>= 769px)
   ═══════════════════════════════════════════════════════════════ */
function moveTikTokBarToHeader() {
    try {
        /* ★ CHỈ CHẠY TRÊN DESKTOP */
        if (window.innerWidth < 769) {
            console.log('ℹ️ Mobile: giữ TikTok bar dưới header');
            return;
        }
        ...
        if (!headerInner || !headerActions) {
            console.log('⚠️ Không tìm thấy .header-inner hoặc .header-actions');
            return;
        }
        if (!tiktokBar) {
            console.log('ℹ️ TikTok bar không tìm thấy hoặc đã ở trong header');
            return;
        }

        /* Di chuyển: chèn trước .header-actions */
        headerInner.insertBefore(tiktokBar, headerActions);

        /* Kiểm tra không che actions sau khi render */
        setTimeout(function() {
            checkHeaderOverflow();
        }, 100);

        console.log('✅ Đã di chuyển TikTok bar vào header (desktop)');
    } catch(e) {
        console.warn('moveTikTokBarToHeader error:', e);
    }
}

/* ═══════════════════════════════════════════════════════════════
   ★ HOÀN TÁC TIKTOK BAR VỀ DƯỚI HEADER — KHI CHUYỂN SANG MOBILE
   ═══════════════════════════════════════════════════════════════ */
function moveTikTokBarBelowHeader() {
    try {
        ...
        /* Di chuyển về sau .header (dưới header) */
        ...
        console.log('✅ Đã đưa TikTok bar về dưới header (mobile)');
    } catch(e) {
        console.warn('moveTikTokBarBelowHeader error:', e);
    }
}

/* ═══════════════════════════════════════════════════════════════
   ★ KIỂM TRA HEADER KHÔNG BỊ TRÀN — LUÔN HIỂN THỊ TIKTOK
   ═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   ★ XỬ LÝ KHI RESIZE — TỰ ĐỘNG DI CHUYỂN
   ═══════════════════════════════════════════════════════════════ */
...
function handleResponsiveTikTok() {
    ...
    if (currentMode === 'desktop') {
        moveTikTokBarToHeader();
    } else {
        moveTikTokBarBelowHeader();
    }
}
...
function populateTikTokFloat() {
    try {
        ...
        var displayName = '';
        if (typeof TIKTOK_NICKNAME !== 'undefined' && TIKTOK_NICKNAME) {
            displayName = TIKTOK_NICKNAME;
        } else if (typeof TIKTOK_USERNAME !== 'undefined' && TIKTOK_USERNAME) {
        ...
        console.warn('populateTikTokFloat error:', e);
    }
}
...
function initApp() {
    ...
    /* ★ Đặt width mode ban đầu + di chuyển TikTok bar (chỉ desktop) */
    _lastWidthMode = window.innerWidth >= 769 ? 'desktop' : 'mobile';
    if (_lastWidthMode === 'desktop') {
        moveTikTokBarToHeader();
    }
...
function updateTierBadge() {
    ...
    if (info.tier === 'trial') {
        ...
        var daysLeft = null;
        /* ★ FIX: guard currentUser undefined */
...
function applyFilter() {
    ...
    /* ★ Auto scroll xuống kết quả khi user chọn filter HSK/chủ đề */
...
function pfApplyFilter() {
    ...
    /* ★ Auto scroll xuống content khi user chọn filter */
    if (state.hsk || state.subject) {
        setTimeout(function() {
