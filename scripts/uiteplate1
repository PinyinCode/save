...
✅ TIKTOK FLOATING BUTTON:
   - Ở góc dưới phải (fixed), không nằm trong header
...
@media (max-width: 768px) {
    ...
    .pf-brand-title { font-size: .78rem; }
}
@media (max-width: 500px) {
    .pf-brand-text { display: none; }
}
...
.pf-tiktok-float::before{
    ...
    inset:-3px;
    border-radius:50%;
    background:conic-gradient(from 0deg,#25f4ee,#fe2c55,#25f4ee,#fe2c55,#25f4ee);
    z-index:-1;
    animation:spinTikTok 4s linear infinite;
    opacity:0;
    transition:opacity .25s;
}
.pf-tiktok-float:hover::before{opacity:1;}
@keyframes spinTikTok{to{transform:rotate(360deg);}}
.pf-tiktok-float:hover{
    transform:scale(1.12) rotate(-6deg);
    box-shadow:
        0 12px 32px rgba(254,44,85,.6),
        0 6px 16px rgba(0,0,0,.4),
        0 0 0 3px rgba(37,244,238,.5);
}
.pf-tiktok-float:active{transform:scale(.95);}
.pf-tiktok-float i{filter:drop-shadow(0 1px 2px rgba(0,0,0,.35));}

@media (max-width:500px){
    .pf-tiktok-float{
        width:44px;height:44px;font-size:1.1rem;
        right:12px;bottom:calc(68px + env(safe-area-inset-bottom));
    }
}
...
<div class="practice-full-modal" id="practiceFullModal">
    ...
    <!-- ★ TikTok floating button ở góc dưới phải -->
...
function initPracticeFull() {
    /* ★ POPULATE TIKTOK FLOATING BUTTON */
    try {
        ...
        if (pfTiktok) {
            ...
            if (tiktokUrl) {
                ...
                var name = (typeof TIKTOK_NICKNAME !== 'undefined' && TIKTOK_NICKNAME)
                    ? TIKTOK_NICKNAME
                    : ((typeof TIKTOK_USERNAME !== 'undefined' && TIKTOK_USERNAME) ? TIKTOK_USERNAME : '');
                if (name) pfTiktok.title = 'TikTok: ' + name;
            } else {
