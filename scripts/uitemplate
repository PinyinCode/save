...
function loadPracticeFull(stt) {
    ...
    // Reset trạng thái nút loa khi đổi câu
...
window.toggleSpeakFull = function() {
    ...
    // Nếu nút đọc nhanh đang chạy → dừng trước
    if (window._isQuickSpeaking) stopQuickSpeak();
...
function _quickSpeakWithKaraoke(token) {
    var phrases = window._pfPhrases || [];
    var charsContainer = $('pfAnswerChars');
    if (!charsContainer || phrases.length === 0) {
        _quickSpeakNormal(token);
        return;
    }
    ...
    function speakNext() {
        if (token !== window._quickSpeakToken) {
            cleanupAll();
            return;
        }
        if (idx >= phrases.length) {
        ...
        setTimeout(function() {
            if (token !== window._quickSpeakToken) { next(); return; }
            try { speechSynthesis.speak(u); } catch(e) { next(); }
        }, 30);
    }
...
function initPracticeFull() {
    ...
    // Bind các nút loa — CHỈ 1 lần duy nhất (tránh đọc 2 lần)
